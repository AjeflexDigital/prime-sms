...
router.get('/sender-ids', async (req, res) => {
  try {
    ...
    const result = await query(`
      SELECT id, sender_id, status, purpose, created_at, approved_at
      FROM sender_ids 
      WHERE user_id = $1
      ORDER BY created_at DESC
    `, [userId]);

    res.json({ senderIds: result.rows });

  } catch (error) {
    console.error('Get sender IDs error:', error);
    res.status(500).json({ message: 'Failed to retrieve sender IDs' });
  }
});

/**
 * POST /api/user/sender-ids
 * Request new sender ID
 */
router.post('/sender-ids', [
  body('senderId').trim().isLength({ min: 3, max: 11 }),
  ...
  try {
    ...
    const existingResult = await query(
      'SELECT id FROM sender_ids WHERE sender_id = $1',
      [senderId.toUpperCase()]
    );

    if (existingResult.rows.length > 0) {
      return res.status(400).json({ message: 'Sender ID already exists' });
    }

    const result = await query(`
      INSERT INTO sender_ids (user_id, sender_id, purpose)
      VALUES ($1, $2, $3)
      RETURNING *
    `, [userId, senderId.toUpperCase(), purpose]);

    res.status(201).json({
      message: 'Sender ID request submitted for approval',
      senderId: result.rows[0]
    });

  } catch (error) {
    console.error('Create sender ID error:', error);
    res.status(500).json({ message: 'Failed to create sender ID request' });
  }
});
