...
export const sendSMS = async ({ to, message, from = 'SMS_PLATFORM' }) => {
  try {
    ...
    if (error.response) {
      ...
      throw new Error(`SMS API Error: ${error.response.data.message || error.response.data || 'Unknown error'}`);
    } else if (error.request) {
      console.error('SMS API No Response:', error.request);
      throw new Error('SMS gateway is unreachable. Check your internet connection and API credentials.');
    } else {
...
const extractNetwork = (phoneNumber) => {
  ...
  const prefix = cleaned.startsWith('234') ? cleaned.substring(3, 6) : cleaned.substring(1, 4);
...
export const handleDeliveryReport = async (data) => {
  try {
    ...
    console.log(`📬 Delivery report received:`, { id, status, networkCode, phoneNumber });
